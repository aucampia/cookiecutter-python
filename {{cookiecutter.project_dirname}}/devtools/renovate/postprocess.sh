...
REPORT_FILE=/tmp/renovate/report.json
...
while IFS= read -r line; do
    ...
    # duplicate review on a long-lived PR. "DISMISSED" is deliberate: the main
    # ruleset has `dismiss_stale_reviews_on_push: false`, so a dismissal is
    # always someone withdrawing an approval by hand, and re-approving it on
    # the next run would merge the PR they were holding back. A repository that
    # does dismiss stale reviews on push wants a human to look anyway.
